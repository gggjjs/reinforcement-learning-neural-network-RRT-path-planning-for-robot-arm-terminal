function uavpath = RRT_WallDis( uavposition, walls, saveTolerance, ...
    objWallDis, sceneRange, stepUnit, objTarget)
...
%% 在节点列表中加入起始节点
newNode = SetRRTNode(uavposition, 0, 0);% 生成RRT的一个点
nodelist = [nodelist newNode];
unExpandNodeIndes = [1];
[cWallDis isCollision]= CalculateUAVWallMinDis(walls, uavposition, saveTolerance);
...
while (currentDis < objWallDis || startTarDis < currentTarDis) && ...
    ...
    [ cWallDis, isCollision ] = ...
        CalculateUAVWallMinDis( walls, newNode.position, saveTolerance);
    ...
    if isCollision == 0
        ...
        hold on
        plot3( [rrtNode1.position(1), rrtNode2.position(1)], ... 
            [rrtNode1.position(2), rrtNode2.position(2)],...
            [rrtNode1.position(3), rrtNode2.position(3)],'g-');
...
for i = 1 : size(uavpath, 1) - 1
    hold on
    plot([uavpath(i, 1), uavpath(i + 1, 1)], ...
        [uavpath(i, 2), uavpath(i + 1, 2)], ...
        [uavpath(i, 3), uavpath(i + 1, 3)],'b-');
end
